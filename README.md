...
that frame, one centered midpoint step with parameter `2t_Q` is the same
passive splitter.  The remaining physical premise is that horizon crossing
selects this centered canonical update; the algebraic and variational maps no
longer need to be separately identified.  See
