...
This is the midpoint form of a single symplectic impulse acting on the
canonical pair already present in the first-order one-graviton action.  Its
Cayley update is exactly lossless and produces the required visible/hidden
amplitudes.  The companion boundary-action file now proves that the same map
is generated by an explicit quadratic type-one boundary functional.
...
## The boundary action and bulk Hamiltonian bridge

The interface has the explicit generating function

```text
F(q_-,q_+) = [q_- q_+ - (d/2)(q_-^2+q_+^2)]/lambda.
```

Lean proves its two canonical variations and proves that they are equivalent
